...
test('Should be able to add a specialty to a user', async ({ page }) => {
  ...
  await page.getByText('La spécialité a été ajoutée', { exact: true }).waitFor({ state: 'visible' })
  await page.reload()
  await page.waitForTimeout(1000)
  await page.getByText('Spécialistes', { exact: true }).click()
...
test("Should be able to remove a user's specialty", async ({ page }) => {
  ...
  await page.waitForTimeout(1000)
  await page.getByText('Spécialistes', { exact: true }).click()
  await expect(userSpecialty.first().getByText('payroll', { exact: true })).toBeHidden()
})
...
test('Should be able, as a specialist, to give my specialty to other users', async ({ page }) => {
  ...
  await addSpecialty.click()
  await expect(userLocator.getByRole('button', { name: 'marketing' })).toBeVisible()
  await expect(userLocator.getByRole('button', { name: 'ce' })).toBeHidden()
