...
module.exports = {
  Query: {
    randomNode: async (_, { tag }, ctx, info) => {
      ...
      if (tag) {
        const { count } = (
          await ctx.prisma.query.tagsConnection(
            {
        ...
        const { count } = (
          await ctx.prisma.query.questionsConnection(
            {},
            `
        {
          aggregate {
        ...
        const randomQuestion = (
          await ctx.prisma.query.questions({ skip: randomIndex, take: 1 }, '{ node { id } }')
        )[0]
