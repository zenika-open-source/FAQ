...
Each service is defined as an **Organizations** with its own configuration. An organization is composed of one or multiple **Groups**.
Those groups are used to separate questions between multiple inherently different scopes.

## How to create a new service/organization
...
You now have a new service and a new group named _"general"_
...
```bash
# Path: ./FAQ/server/
npm run new_group [service_name] [service_stage] [group_name]

# Example: npm run new_group zenika pre-prod "fonctions support"
```
