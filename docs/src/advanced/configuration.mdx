...
If a user has the `admin` attribute set to `true`, he will have access to a **Settings page** where he will be able to manually change the `tags` and `algoliaSynonyms` values.
